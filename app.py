...
# =========================================================
# CONFIG & LOGGING (FIXED FOR RENDER STDOUT)
# =========================================================
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s - %(levelname)s - %(message)s",
    handlers=[logging.StreamHandler()]  # Streams directly to Render dashboard logs
)
logger = logging.getLogger(__name__)
...
def compute_segmentation_results(image_np, pred_mask, threshold, overlay_alpha, gt_mask=None):
    # FIXED: Dynamic handling based on model output dimension instead of hardcoded 384
    mask_model_size = (pred_mask > threshold).astype(np.uint8)
    ...
    if tumor_area > 0:
        confidence = float(pred_mask[mask_model_size == 1].mean())
    else:
        confidence = 0.0
...
async def api_upload(
    ...
    try:
        ...
        with torch.no_grad():
            output = model(input_tensor)
            prob = torch.sigmoid(output)
            pred_mask = prob.squeeze().cpu().numpy()
        inference_time = (time.time() - start_time) * 1000  # ms
        
        cam_mask = grad_cam.generate(input_tensor)
        ...
        overlay, stats = compute_segmentation_results(
            image_np, pred_mask, 0.5, 0.5, gt_mask
        )
        stats["inference_time"] = round(inference_time, 2)
        
        cam_resized = cv2.resize(cam_mask, (image_np.shape[1], image_np.shape[0]))
        cam_resized = np.clip(cam_resized, 0, 1)
        cam_uint8 = np.uint8(cam_resized * 255)
        heatmap = cv2.applyColorMap(cam_uint8, cv2.COLORMAP_JET)
        original_bgr = cv2.cvtColor(image_np, cv2.COLOR_GRAY2BGR)
...
async def api_update(req: UpdateRequest):
    try:
        ...
        image_np = session["image_np"]
        pred_mask = session["pred"]
        cam_mask = session["cam"]
        gt_mask = session["gt_mask"]
        
        overlay, stats = compute_segmentation_results(
            image_np, pred_mask, req.threshold, req.overlay_alpha, gt_mask
        )
